...
figure; subplot(2,1,1); plot(dataL_aligned(:,1:3));title('left side');
subplot(2,1,2); plot(dataR_aligned(:,1:3));title('right side');
...
function out = calibrate_aligned(in, wlen, threshold, test_ind)
...
% strip gravity from acc reading
    ...
    out.left.acc = acc(test_ind(1):test_ind(2), :);
    out.left.gyr = gyr(test_ind(1):test_ind(2), :);
    out.left.quat = quat(test_ind(1):test_ind(2), :);
    out.left.acc_g = acc_g(test_ind(1):test_ind(2), :);
    ...
    out.right.acc = acc(test_ind(1):test_ind(2), :);
    out.right.gyr = gyr(test_ind(1):test_ind(2), :);
    out.right.quat = quat(test_ind(1):test_ind(2), :);
    out.right.acc_g = acc_g(test_ind(1):test_ind(2), :);
